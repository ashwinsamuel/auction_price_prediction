...
path = '/Users/ashwinsamuel/Documents/Kaggle/closing_auction_prediction/202503_imbalance/'
WINDOW_MULTIPLIER = 4
...
def get_target(df):
    ...
    return df[['final_cross', 'ref_price']]
...
def generate_features(df, window_multiplier):
    ...
    print(f"We got total {len(features)} Features. {datetime.now().time()}")
    df = pd.concat([df] + all_new_features, axis=1)
    print(f'here 1. {datetime.now().time()}. {len(df.columns)}')
    df = df[features]
    print(f'here 3. {datetime.now().time()}')
    df = df.dropna(subset='ref_price')
    print(f'here 4. {datetime.now().time()}')
...
CV_fold = 5
model_dict = {
    'lgb': lgb.LGBMClassifier(objective='binary', metric=['binary_logloss', 'binary_error'], n_estimators=1000),
    'xgb': xgb.XGBClassifier(objective='binary:logistic', eval_metric=['logloss', 'error'], n_estimators=1000),
    'cbt': cbt.CatBoostClassifier(eval_metric='Logloss', custom_metric=['Accuracy'], n_estimators=1000)
}
...
if __name__=='__main__':
    ...
    compiled_df = load_data(path)
    
    target = get_target(compiled_df)
    
    features = generate_features(compiled_df, WINDOW_MULTIPLIER)
    
    # clear 0
    filter_condition = target['final_cross'] != target['ref_price']
    print(f'here 5. {datetime.now().time()}')
    features.index = target.index
    print(f'here 6. {datetime.now().time()}')
    features = features[filter_condition]
    print(f'here 7. {datetime.now().time()}')
    target = target[filter_condition]
    print(f'here 8. {datetime.now().time()}')
    
    X = features.drop(columns=['symbol', 'local_time', 'date', 'cross']).values
    Y = np.where( target['final_cross']>target['ref_price'], 1, 0) # check type is np array
    
    split = int(0.8*len(X))
    X_train, Y_train = X[:split], Y[:split]
    X_test, Y_test = X[split:], Y[split:]
    
    model1 = train(X_train, Y_train, X_test, Y_test, model_name='lgb')
    model2 = train(X_train, Y_train, X_test, Y_test, model_name='xgb')
    model3 = train(X_train, Y_train, X_test, Y_test, model_name='cbt')
    
    # os.chdir('/Users/ashwinsamuel/Documents/Kaggle/closing_auction_prediction/models/n1000_classification_with_mul')
    # model.booster_.save_model(f'model_mul{WINDOW_MULTIPLIER}.txt')
    
    os.chdir('/Users/ashwinsamuel/Documents/Kaggle/closing_auction_prediction/saved_features/')
    features = pd.read_feather(f"features_251_mul6.feather")
